async function fetchCheapestFlight(departure_city, arrival_city, departure_date, flight_class, requested_currency) {
...
document.getElementById("flight-search-form").addEventListener("submit", async (event) => {
    ...
    const data = await fetchCheapestFlight(departure_city, arrival_city, departure_date, flight_class, requested_currency);
    ...
    output.textContent = `The cheapest flight from ${departure_city} to ${arrival_city} departing on ${departure_date}, under ${flight_class.toUpperCase()} Class is ${requested_currency}${price} on ${airline}`;
